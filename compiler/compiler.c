...
static int
recursivelyParseGlobalContent(astNodePtr tree, compilationContext *ctx);
static int
addImport(const char *name, compilationContext *ctx);
static int
importModuleFromDirectory(const char *directory, const char *name, compilationContext *ctx);
static int
addSubmodule(compilationCtx *ctx, plModule *submodule);
static int
addExport(const char *name, compilationContext *ctx);
static int
parseStruct(astNodePtr tree, compilationContext *ctx);
static int
parseNamedSource(astNodePtr tree, compilationContext *ctx);
static int
parseNamedPipe(astNodePtr tree, compilationContext *ctx);
static int
parseNamedSink(astNodePtr tree, compilationContext *ctx);
static void
pushContextToStack(int nodeType, compilationContext *ctx);
static int
popContextFromStack(compilationContext *ctx);
static const variableReference *
resolveReference(const char *name, size_t len);
...
recursivelyParseGlobalContent(astNodePtr tree, compilationContext *ctx)
{
    ...
    case IMPORT: return addImport((const char *)tree->first->first, ctx);

    case EXPORT: return addExport((const char *)tree->first->first, ctx);

    case STRUCT: return parseStruct(tree, ctx);

    case SOURCE: return parseNamedSource(tree, ctx);

    case PIPE: return parseNamedPipe(tree, ctx);

    case SINK: return parseNamedSink(tree, ctx);

    case MAIN: return parseMain(tree, ctx);

    default: ERROR_QUIT("Invalid nodeType: %i\n", tree->nodeType);
    }
}
...
pushContextToStack(int nodeType, compilationContext *ctx)
{
    if (ctx->stack.length >= ctx->stack.capacity) {
        ...
        newCapacity =
            (ctx->stack.capacity == 0) ? STACK_INITIAL_CAPACITY : (ctx->stack.capacity + 1) * 5 / 4;
