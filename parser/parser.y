...
%token EOF ARROW LSHIFT RSHIFT AND OR
%token <node> NAME
%token <marker> TYPE COMPARISON OPERATOR_ASSIGNMENT CONTEXT
%token <object> LITERAL

%type <node> file file_body global_content main_definition import_statement export_statement
...
file: file_body EOF {$$=$1;};

file_body: {$$=NULL;}
	| file_body global_content {$$ = createTwoSplitNode('F',$1,$2);};
